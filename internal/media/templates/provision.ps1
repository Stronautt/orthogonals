...
$shares = @(
{{- range .Shares}}
    @{ Service = '{{ps .Service}}'; Tag = '{{ps .Tag}}'; Drive = '{{ps .Drive}}' }
{{- end}}
)
...
{{- if .Shares}}
    @{
        # VirtioFsSvc mounts exactly one device and takes its tag from the
        # service command line, never its registry key: the first share
        # reconfigures the shipped service, the rest get clones of it. Untagged,
        ...
        # under "Program Files", so the binPath carries both quotes and spaces,
        # and Windows PowerShell wraps such an argument in one more pair of
        # quotes without escaping the inner ones - sc.exe then reads a path that
        ...
        Run  = {
            ...
            foreach ($s in $shares) {
                ...
                $key = $servicesKey + $s.Service
                if (Get-Service -Name $s.Service -ErrorAction SilentlyContinue) {
                    # a command line only takes effect on the next start
                    Stop-Service -Name $s.Service -Force -ErrorAction SilentlyContinue
                } else {
                    New-Service -Name $s.Service -BinaryPathName $bin -DisplayName ('VirtIO-FS ' + $s.Tag) | Out-Null
                }
